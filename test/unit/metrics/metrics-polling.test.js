...
describe('metrics-polling', () => {
  ...
  describe('startMetricsPolling', () => {
    ...
    test('should call queue.enqueue immediately on start', () => {
      startMetricsPolling()
    ...
    test('should call queue.enqueue periodically in development', () => {
      config.isDev = true
      config.metricsPollingInterval = 60000
      startMetricsPolling()
    ...
    test('should handle scheduled calculation failure in development', async () => {
      config.isDev = true
      config.metricsPollingInterval = 60000
      metricsQueue.enqueue.mockResolvedValueOnce()
      metricsQueue.enqueue.mockRejectedValueOnce(new Error('Calculation failed'))
      startMetricsPolling()
      jest.advanceTimersByTime(60000)
      await Promise.resolve()
